...
import { User as UserType } from '@/src/app/types/feed';
...
export default function PhotoEditorModal({ isOpen, onClose, user }: PhotoEditorModalProps) {
    ...
    <>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className={`p-3 rounded-full transition-colors duration-200 ${
                      rightPanelView === 'edit' ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100 text-slate-600'
                    }`}
                  ...
                  <button
                    ...
                    className={`p-3 rounded-full transition-colors duration-200 ${
                      rightPanelView === 'mention' ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100 text-slate-600'
                    }`}
                ...
                <div className="flex border-b border-slate-200 mb-6">
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
                      editTab === 'crop' ? 'text-slate-800 border-blue-500' : 'text-slate-500 border-transparent hover:text-slate-700'
                    }`}
                    aria-label="Crop tab"
                  >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
                      editTab === 'filter' ? 'text-slate-800 border-blue-500' : 'text-slate-500 border-transparent hover:text-slate-700'
                    }`}
                  ...
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
                      editTab === 'adjust' ? 'text-slate-800 border-blue-500' : 'text-slate-500 border-transparent hover:text-slate-700'
                    }`}
                        ...
                        {filters.map((filter) => (
                          <button
                            ...
                            className={`relative p-3 rounded-lg border-2 text-center transition-all ${
                              selectedFilter === filter.value ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'
                            }`}
            ...
            {rightPanelView === 'mention' && (
              <div className="w-96 border-l border-slate-200 pl-6">
                ...
                <div className="mt-6 pt-4 border-t border-slate-200">
                  ...
                  {taggedUsers.length > 0 ? (
                    taggedUsers.map((user) => (
                      <div key={user.id} className="flex items-center gap-2 p-2 bg-blue-50 rounded-lg mb-2">
                        <img src={user.avatar} alt={`${user.name}'s avatar`} className="w-6 h-6 rounded-full" />
                        <span className="text-xs text-slate-700 flex-1 truncate">{user.name}</span>
                        <button
                          onClick={() => removeUserTag(user.id)}
      ...
      {showAltTextModal && (
        <AltTextModal
          ...
          altText={altText}
          setAltText={(text) => {
            updateImageTransform({ altText: text });
            setAltText(text);
          }}
        />
      )}
