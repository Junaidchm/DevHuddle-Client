...
const getProfileImageUrl = (pic?: string | null) =>
  pic && pic !== "null" ? `http://localhost:8080/auth/${pic}` : PROFILE_DEFAULT_URL;
     ...
     <>
      <nav className="bg-white/95 backdrop-blur-md shadow sticky top-0 z-[100] px-6 py-3 flex justify-between items-center">
             <div className="logo flex items-center font-bold text-xl text-indigo-600">
               <span className="bg-gradient-to-r from-indigo-600 to-cyan-500 bg-clip-text text-transparent">Dev</span>Huddle
             </div>
             <div className="nav-links flex gap-6 items-center">
               
               <NavLink href="community-feed.html" label="Community Feed" isActive={true} />
               <NavLink href="domain-hubs.html" label="Domain Hubs" isActive={false} />
               <NavLink href="events-hackathons.html" label="Events" isActive={false} />
               <NavLink href="portfolios.html" label="Portfolios" isActive={false} />
               <NavLink href="submit-project.html" label="Submit Project" isActive={false} />
             </div>
             {user? <Link href={`/profile/update/${user.username}`}>
