...
export default function PostsPage() {
  ...
  const [hideModal, setHideModal] = useState<{ isOpen: boolean; postId: string | null }>({
    isOpen: false,
    postId: null,
  });
  ...
  return (
    <div className="p-6 w-full h-full">
      <Card>
        <CardHeader title="Posts Management">
          <SearchInput
          ...
          <table className="w-full border-collapse min-w-[1000px]">
            <thead>
              <tr>
                ...
                <th className="p-4 bg-gray-100 text-gray-500 font-semibold text-xs uppercase tracking-wider text-left">Post Content</th>
                ...
                <tr>
                  <td colSpan={7} className="p-12 text-center text-gray-500">
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <span className="font-medium">Fetching posts...</span>
                    </div>
                  </td>
                </tr>
              ) : error ? (
                <tr>
                  <td colSpan={7} className="p-12 text-center">
                    <div className="bg-red-50 border border-red-200 rounded-lg p-6 inline-block max-w-md">
                      ...
                      <p className="text-red-800 font-bold">Error loading posts</p>
                  ...
                  >
                    <td className="p-4">
                      <div className="relative inline-block">
                        ...
                        <label htmlFor={`post-${post.id}`} className="w-[18px] h-[18px] border border-gray-300 rounded-sm flex items-center justify-center cursor-pointer transition-all duration-300 peer-checked:bg-indigo-600 peer-checked:border-indigo-600">
                          <i className="fas fa-check text-white text-[0.625rem] hidden peer-checked:inline-block"></i>
                        </label>
                      </div>
                    </td>
                    ...
                    <td className="p-4">
                      <span className={`text-sm font-semibold ${post.reportsCount > 0 ? "text-orange-600" : "text-gray-600"}`}>
                        {post.reportsCount || 0}
                      </span>
                    </td>
                    <td className="p-4">
                        ...
                        {!post.deletedAt && (
                          <>
                            {post.isHidden ? (
                              ...
                              <button
                                onClick={() => setHideModal({ isOpen: true, postId: post.id })}
      ...
      <ConfirmModal
        ...
        onClose={() => setHideModal({ isOpen: false, postId: null })}
        onConfirm={(reason) => {
          if (hideModal.postId) {
            hideMutation.mutate({ postId: hideModal.postId, hidden: true, reason });
          }
        }}
        title="Hide Post"
        message="Are you sure you want to hide this post? It will no longer be visible to users but will remain in the database."
      ...
      <ConfirmModal
        ...
        title="Delete Post"
        message="This action is irreversible. Are you sure you want to permanently delete this post and all its associated data?"
