...
export default async function FullPostsPage({ params }: { params: { user_name: string } }) {
    ...
    <div className="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-4 mb-6">
            <Link href={`/profile/${user_name}`}>
             ...
             <div>
                <h1 className="text-2xl font-bold leading-tight">All Posts</h1>
                <p className="text-muted-foreground text-sm">from {user.name}</p>
            </div>
        </div>
        
        <div className="space-y-4">
             <ActivitySection type="posts" userId={user.id} currentUserId={currentUserId} />
        </div>
    </div>
  );
}
