...
import { queryKeys } from "../components/profile/queryKeys";
  ...
  const followMutation = useMutation({
    ...
    onMutate: async (userId: string) => {
      ...
      const profileQueryKey = queryClient
        .getQueryCache()
        .getAll()
        .find((query) => query.getObserversCount() > 0 && query.queryKey[0] === 'profiles' && (query.state.data as UserProfile)?.id === userId)
        ?.queryKey;
    ...
    onError: (error: any, _, context) => {
      if (context?.previousProfile) {
        queryClient.setQueryData(context.profileQueryKey, context.previousProfile);
      }
